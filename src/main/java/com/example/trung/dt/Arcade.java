...
public class Arcade extends AppCompatActivity implements GameMode {
    ...
    public void nextLevel() {
        ...
        calc.setBlockedCoords(new float[calc.calcDots(level)][2]);

        for (int i = 0; i < calc.calcDots(level); i++) {
    ...
    public void dotClicked(View dot) {
        ...
        if (dotsClicked == calc.calcDots(level)) {
            timer.stopThread();
            nextLevel();
        }
    }
